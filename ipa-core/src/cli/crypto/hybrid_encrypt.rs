...
use crate::{
    cli::playbook::{BreakdownKey, InputSource, TriggerValue},
...
impl EncryptArgs {
    ...
    pub fn hybrid_encrypt(&self) -> Result<(), BoxError> {
...
mod tests {
    ...
    use crate::{
        ...
        test_fixture::{hybrid::TestHybridRecord, join3v, Reconstruct, TestWorld},
    };

    #[tokio::test]
    async fn hybrid_encrypt_and_execute_query() {
    ...
    fn hybrid_encrypt_no_network_file() {
    ...
    fn hybrid_encrypt_bad_network_file() {
        let input_file = sample_data::write_csv(sample_data::test_ipa_data().take(10)).unwrap();
        let output_dir = tempdir().unwrap();
        let network_data = r"
this is not toml!
...
";
    ...
    fn hybrid_encrypt_incomplete_network_file() {
        let input_file = sample_data::write_csv(sample_data::test_ipa_data().take(10)).unwrap();
